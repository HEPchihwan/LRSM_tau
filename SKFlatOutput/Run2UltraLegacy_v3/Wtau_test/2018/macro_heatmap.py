...
def plot_heatmaps():
    ...
    # 2D 배열 초기화
    taucut_ratios_matrix = np.zeros((len(N_values),len(WR_values)))
    mutaucut_ratios_matrix = np.zeros((len(N_values),len(WR_values)))
    ratio_matrix = np.zeros((len(N_values),len(WR_values)))
    ...
    masked_data0 = np.ma.masked_equal(taucut_ratios_matrix, 0.0)  # 값이 0보다 작은 부분 마스킹
    masked_data1 = np.ma.masked_equal(mutaucut_ratios_matrix, 0.0)  # 값이 0보다 작은 부분 마스킹
    masked_data2 = np.ma.masked_equal(ratio_matrix, 0.0)  # 값이 0보다 작은 부분 마스킹
    # 히트맵 그리기 함수
    def draw_heatmap(data, filename, cmap='plasma'):
        ...
        cbar.set_label('Improvment (%)',fontsize=40)
