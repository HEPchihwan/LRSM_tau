...
while WRMASS <= WRMASSEND:
    ...
    while NMASS < WRMASS:
        ...
        os.system(f"cp skeletons_for_NLO_Nother/run_card.dat {DIRNAME}/{DIRNAME}_run_card.dat")
        os.system(f"cp skeletons_for_NLO_Nother/extramodels.dat {DIRNAME}/{DIRNAME}_extramodels.dat")

        with open("skeletons_for_NLO_Nother/proc_card.dat", "r") as f:
            proclines = f.readlines()

        with open("skeletons_for_NLO_Nother/customizecards.dat", "r") as f:
